...
if(isset($_POST["submit"])){
	...
	$result=mysqli_query($con,"INSERT INTO `airport_tb`( `airport`,`location`) VALUES ('$airport','$location')");
	}
?>
...
<div class="container-fluid">
	...
	<div class="col-lg-12">
		<div class="row">
			<!-- FORM Panel -->
			<div class="col-md-4">
			<form action="airport.php" method="POST" id="airport" enctype='multipart/form-data'>
				<div class="card">
					...
					<div class="card-body">
							...
							<div class="form-group">
								...
								<textarea name="airport" id="airport" name="airport" cols="30" rows="2" class="form-control"></textarea>
							</div>
							<div class="form-group">
								<label for="" class="control-label">Location</label>
								<textarea name="location" id="location" name="location" cols="30" rows="2" class="form-control"></textarea>
							</div>
							<div class="form-group">
								<img src="" alt="" id="cimg">
							</div>	
					...
					<div class="card-body">
						<table class="table table-bordered table-hover">
							...
							<tbody>
								<?php 
								$i = 1;
								$cats = $con->query("SELECT * FROM airport_tb order by id asc");
								while($row=$cats->fetch_assoc()):
								?>
								<form action="airport.php" method="POST" enctype='multipart/form-data'>
								<tr>
									<td class="text-center"><?php echo $i++ ?><input name="edit_sr" type="hidden" value="<?php echo $i++ ?>"></td>
									<!--<td class="text-center">-->
									<td>
										<?php echo $row['airport']?></b>
									</td>
									...
									<td class="text-center">
										<!--<button class="btn btn-sm btn-primary edit_airline" name="edit_submit" type="submit" onclick="" data-id="<?php echo $row['id'] ?>" data-airlines="<?php echo $row['airlines'] ?>" data-logo_path="<?php echo $row['image'] ?>" >Edit</button>-->
										<button class="btn btn-sm btn-danger delete_airport" name="delete_submit" type="button" onclick="delete_airport(<?php echo $row['id']; ?>);" data-id="<?php echo $row['id'] ?>">Delete</button>
									</td>
								</tr>
								</form>
								<?php endwhile; ?>
...
<script>
	...
	function delete_airport($id){
		// start_load()
		$.ajax({
			url:'airport_class.php',
			method:'POST',
			data:{id:$id,action:"delete_airport"},
			success:function(resp){
				alert(resp);
